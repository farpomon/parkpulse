...
**Coverage: 65 parks across 5 regions** (incl. every major trackable US chain — Six Flags, legacy Cedar Fair, SeaWorld/Busch, LEGOLAND, and independents like Silver Dollar City, Kennywood, Holiday World, Lagoon) — Florida (WDW ×4, Universal Orlando
...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | Enables **Continue with Google**. Create an OAuth 2.0 Web application client in the [Google Cloud console](https://console.cloud.google.com/apis/credentials) and register `https://<host>/api/auth/oauth/google/callback` as an authorised redirect URI — once per host you deploy (production, dev, and `http://localhost:3000/...` for local work). Either variable missing = the button never appears and the route 404s. |
| `APPLE_CLIENT_ID` / `APPLE_TEAM_ID` / `APPLE_KEY_ID` / `APPLE_PRIVATE_KEY` | Enables **Continue with Apple**, and needs a paid Apple Developer account. `APPLE_CLIENT_ID` is the **Services ID** (not the app bundle id), `APPLE_TEAM_ID` is the 10-character team id, and `APPLE_KEY_ID`/`APPLE_PRIVATE_KEY` are the id and full contents of a Sign In with Apple `.p8` key. Paste the key with real newlines or with `\n` escapes — both are accepted. Register the same `/api/auth/oauth/apple/callback` URL as a Return URL on the Services ID; Apple requires HTTPS, so it cannot be exercised against localhost. All four missing or partial = the button never appears. |
| `OAUTH_REDIRECT_BASE` | Overrides the origin used to build those callback URLs. Normally unnecessary — it is derived from the request — but a proxy that rewrites the host will need it (e.g. `https://www.parkpulse.fun`). |
| `AI_BUDGETS` | JSON overriding the per-plan daily AI budget in dollars, e.g. `{"week-pass":1.5}`. Defaults: Day Pass `2.50`, Week/Trip `1.00`, Month `0.50`, 6-Month `0.40`, Annual `0.35`, comp `0.50`, dev `25`. Budgets are in **dollars, not questions** — a question that makes Mila use a tool bills several times over, so counting questions never capped the bill. |
| `AI_BUDGET_FREE` | Daily AI budget for an account with no active pass (default `0.20`) — enough for the one free plan review. |
| `AI_GLOBAL_DAILY_USD` | The backstop across every account (default `50`). Past it, AI features say Mila is resting; plans, waits, forecasts, board and map keep working, since none of them need a model. `0` disables it. |
| `STRIPE_PRICE_MILA_TOPUP` / `MILA_TOPUP_USD` / `MILA_TOPUP_LABEL` | Sells more of Mila's time to a visitor who has used their day's budget. The Stripe price is what they pay; `MILA_TOPUP_USD` (default `2.00`) is the model credit granted, spent before the daily allowance and carried across days. Unset price = the offer never appears. |
| `AI_ALERT_USD` | Immediate alert when a day's AI spend crosses this many dollars (default `25`; `0` disables the ceiling). Sent to `AI_REPORT_TO`, at most once a day, so a runaway loop does not become a mail flood on top of a bill. |
| `AI_ALERT_MULTIPLE` / `AI_ALERT_FLOOR_USD` | The other alert: fires when today runs at least `AI_ALERT_MULTIPLE`× the trailing-week daily average (default `4`), but only once the day is above `AI_ALERT_FLOOR_USD` (default `5`) — below that the multiple is noise, since 4× of eleven cents is not news. |
